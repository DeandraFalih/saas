...
class Application(tk.Tk):
    def __init__(self):
        ...
        # Create label for displaying API status for first camera
        self.status_label1 = tk.Label(self.left_frame, text="API Status (Cam 1): ", font=("Helvetica", 12))
        self.status_label1.pack(padx=10, pady=10)
        ...
        # Create label for displaying API status for second camera
        self.status_label2 = tk.Label(self.right_frame, text="API Status (Cam 2): ", font=("Helvetica", 12))
        self.status_label2.pack(padx=10, pady=10)
        ...
        self.cap2 = cv2.VideoCapture(3)
        
        # Variables to store detected plates
        self.detected_plate1 = ""
        self.detected_plate2 = ""

        self.update_frame()
        self.send_to_api_periodically()
    ...
    def update_frame(self):
        ...
        if ret1:
            ...
            self.detected_plate1 = self.process_frame(frame1, "Camera 1")
            ...
            self.plate_label1.config(text=f"Detected License Plate (Cam 1): {self.detected_plate1}")
        ...
        if ret2:
            ...
            self.detected_plate2 = self.process_frame(frame2, "Camera 2")
            ...
            self.plate_label2.config(text=f"Detected License Plate (Cam 2): {self.detected_plate2}")
    ...
    def send_to_api(self, url, license_plate):
        try:
            response = requests.post(url, json={"license_plate": license_plate})
            response.raise_for_status()
            return "Success"
        except requests.exceptions.RequestException as e:
            return f"Failed: {e}"

    def send_to_api_periodically(self):
        if self.detected_plate1:
            status1 = self.send_to_api("https://harezayoankristianto.online/api/parkings/in", self.detected_plate1)
            self.status_label1.config(text=f"API Status (Cam 1): {status1}")

        if self.detected_plate2:
            status2 = self.send_to_api("https://harezayoankristianto.online/api/parkings/out", self.detected_plate2)
            self.status_label2.config(text=f"API Status (Cam 2): {status2}")

        self.after(1000, self.send_to_api_periodically)  # Schedule this method to be called every 1 second
